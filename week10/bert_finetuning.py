...
with torch.no_grad():
    for batch in test_loader:
        ...
        predictions = torch.argmax(logits, dim=1)  # pick the highest score
...
def predict(text):
    ...
    with torch.no_grad():
        outputs = backbone(input_ids=tokens["input_ids"], attention_mask=tokens["attention_mask"])
        cls_vector = outputs.last_hidden_state[:, 0, :]
        logits = classifier(cls_vector)
...
print(predict("NASA launches new satellite into orbit"))
print(predict("Manchester United wins the Champions League"))
print(predict("Apple stock surges after record earnings"))
